"""
Sporthive API interface for fetching activity data
"""
import math
import random
import time
...
from src.utils.helpers import to_float, floor2, get_headers
...
def get_bestlap_info_for_activity(
    ...
    best_time_s = to_float(best.get("duration"))
    lap_speed_kph = floor2((best.get("speed") or {}).get("kph") or 0.0)
    ...
    def sec_vals(sec):
        if not sec:
            return None, None
        dur = to_float(sec.get("duration"))
        spd = (sec.get("speed") or {}).get("kph")
        return dur, floor2(spd) if spd is not None else None
